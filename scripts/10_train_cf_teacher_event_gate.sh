...
OUTPUT_ROOT="${OUTPUT_ROOT:-/root/autodl-tmp/cer-rec/beauty/event_gate_cf_teacher}"
for lambda_route in 0.1 0.3 1.0; do
  out_dir="${OUTPUT_ROOT}/lambda_${lambda_route}"
  ...
  "$PYTHON_BIN" -m src.evaluation.event_gate_analysis \
    ...
    --learned-label "EventGate-CFTeacher-lambda-${lambda_route}"
done
